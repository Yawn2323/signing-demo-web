...
export class SignaturePad {
    ...
    constructor(public canvas: HTMLCanvasElement, private options: IOptions = {}) {
    ...
    private _strokeUpdate(event: MouseEvent | Touch): void {
        const [x, y] = this.computeTouchPosition(event);
    ...
    // Custom stuff
    // To adopt canvas size, scale, touch point, device dpr
    ...
    computeTouchPosition(event: MouseEvent | Touch) {
        this.calOffset();

        // Click or touch screen pos
        const screenX = event.clientX;
        const screenY = event.clientY;

        // Moved by canvas offset to screen
        const pageX = screenX - this.offsetX;
        const pageY = screenY - this.offsetY;

        // Scaled by canvas view width
        const canvasX = (pageX / this.scale);
        const canvasY = (pageY / this.scale);
